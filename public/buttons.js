...
$(document).ready(function() {
  ...
  Promise.all([
    ...
    for (var idx = 0; idx < 10; idx++) {
      new Path.Rectangle({
        point: [vssw*1.5, (keysize + 5) * idx + vssw * 4.5],
        size: [vssw*7, keysize],
        fillColor: 'white',
        _restfill: 'white',
    ...
    for (; idx < 2; idx++) {
      new Path.Rectangle({
        point: [vssw*1.0 + vssw * -0.1, (keysize + 7) * idx + vssw * 4.5 + keysize / 2],
        size: [vssw*3.7, keysize2],
        fillColor: '#444'
      });
    }
    for (; idx < 5; idx++) {
      new Path.Rectangle({
        point: [vssw*1.0 + vssw * -0.1, (keysize + 7) * idx + vssw * 4.5 + keysize / 2 + keysize],
        size: [vssw*3.7, keysize2],
        fillColor: '#444'
      });
    }
    for (; idx < 7; idx++) {
      new Path.Rectangle({
        point: [vssw*1.0 + vssw * -0.1, (keysize + 7) * idx + vssw * 4.5 + keysize / 2 + keysize * 2],
        size: [vssw*3.7, keysize2],
        fillColor: '#444'
      });
    }
